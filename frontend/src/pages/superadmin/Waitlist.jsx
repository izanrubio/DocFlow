...
import { getSuperAdminWaitlist, getSuperAdminWaitlistStats, launchSuperAdminWaitlist } from '../../api/superadmin';
...
function exportCSV(entries) {
    ...
    const rows   = entries.map((e) =>
        [`"${(e.name ?? '').replace(/"/g, '""')}"`, `"${e.email}"`, `"${e.source ?? ''}"`, `"${new Date(e.created_at).toLocaleString('es-ES')}"`].join(',')
    );
...
export default function SuperAdminWaitlist() {
    ...
    const { data: stats, isLoading: statsLoading } = useQuery({
        queryKey: ['superadmin-waitlist-stats'],
        queryFn:  () => getSuperAdminWaitlistStats().then((r) => r.data.data),
    });

    const { data: list, isLoading: listLoading } = useQuery({
        queryKey: ['superadmin-waitlist-list', page],
        queryFn:  () => getSuperAdminWaitlist(page).then((r) => r.data.data),
        keepPreviousData: true,
    });

    const launchMutation = useMutation({
        mutationFn: launchSuperAdminWaitlist,
        onSuccess:  (res) => { toast.success(`Enviando emails a ${res.data.data.total_emails} personas.`); setShowLaunch(false); },
        onError:    (err) => { toast.error(err.response?.data?.message ?? 'No se pudo lanzar.'); setShowLaunch(false); },
    });
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1 text-xs border rounded-lg hover:bg-gray-50 disabled:opacity-40">← Anterior</button>
                                <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1 text-xs border rounded-lg hover:bg-gray-50 disabled:opacity-40">Siguiente →</button>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500 mb-4">Envía el email de lanzamiento a todos los emails de la lista. Esta acción no se puede deshacer.</p>
                            <button
